...
test('rewrites old repo-prefixed managed markers to the repo-neutral markers', () => {
  const body = `<!-- kitcn:auto-release:start -->
- [x] Auto release
<!-- kitcn:auto-release:end -->
...
`;
  ...
  assert.doesNotMatch(nextBody, /kitcn:auto-release/);
  assert.match(nextBody, /- \[x\] Auto release/);
});
