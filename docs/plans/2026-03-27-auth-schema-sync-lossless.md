...
## Goal
Keep `kitcn add auth --only schema` lossless for `example/`, including
the `--overwrite` path when auth tables are already explicitly owned locally.

## Findings
- Plain `bunx kitcn add auth --only schema --yes` in `example/` is
  already lossless; it skips `schema.ts` and `plugins.lock.json`.
- `bunx kitcn add auth --only schema --overwrite --yes` is not
...
- Root cause is `decideOwnership()` in
  `packages/kitcn/src/cli/registry/schema-ownership.ts`:
...
- Live `example/` proof passed:
  - `bunx kitcn add auth --only schema --overwrite --yes`
