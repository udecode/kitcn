...
type ProcedureExport = {
  ...
  isMutation?: boolean;
  isAction?: boolean;
  __kitcnTransformer?: DataTransformerOptions;
  __kitcnRawHandler?: ProcedureRawHandler;
};
...
async function executeProcedure(
  ...
  if (mode === 'handler') {
    if (typeof procedure.__kitcnRawHandler === 'function') {
      return procedure.__kitcnRawHandler({ ctx, input });
    }
    ...
    throw new Error(
      `[kitcn] Resolved procedure does not expose a raw handler: "${pathString}".`
    );
  }
...
function assertCanInvoke(
  ...
  if (ctxType === 'action') {
    if (!opts.supportsActionContext) {
      throw new Error(
        `[kitcn] Action context is not supported by createCaller(ctx): "${pathString}".`
      );
    }
    if (procedureType === 'action') {
      throw new Error(
        `[kitcn] Cannot call action procedures from action context: "${pathString}".`
      );
    }
    return;
  }
  ...
  if (ctxType !== 'query' && ctxType !== 'mutation') {
    throw new Error(
      `[kitcn] Unsupported context for createCaller(ctx): "${pathString}".`
    );
  }

  if (ctxType === 'query' && procedureType !== 'query') {
    throw new Error(
      `[kitcn] Cannot call ${procedureType} procedures from query context: "${pathString}".`
    );
  }

  if (ctxType === 'mutation' && procedureType === 'action') {
    throw new Error(
      `[kitcn] Mutation context cannot call action procedures: "${pathString}".`
    );
  }
}
  ...
  return new Proxy(() => {}, {
    ...
    async apply(_target, _thisArg, argsList) {
      ...
      if (!isProcedureDefinition(node)) {
        throw new Error(
          `[kitcn] Path does not resolve to a procedure: "${pathString}".`
        );
      }
      ...
      if (!isProcedureExport(resolved)) {
        throw new Error(
          `[kitcn] Resolved value is not a cRPC procedure: "${pathString}".`
        );
      }
      ...
      if (resolvedType !== node._type) {
        throw new Error(
          `[kitcn] Procedure type mismatch at "${pathString}". Expected "${node._type}" but got "${resolvedType ?? 'unknown'}".`
        );
      }
...
async function executeActionContextProcedure(
  ...
  if (!functionReference) {
    throw new Error(
      `[kitcn] Missing function reference metadata for action context dispatch: "${pathString}".`
    );
  }
  ...
  if (typeof runner !== 'function') {
    ...
    throw new Error(
      `[kitcn] Action context is missing ctx.${runnerName} for "${pathString}".`
    );
  }
...
async function executeActionContextActionProcedure(
  ...
  if (typeof ctxValue.runAction !== 'function') {
    throw new Error(
      `[kitcn] Action context is missing ctx.runAction for "${pathString}".`
    );
  }
...
function getScheduleRunner(
  ...
  if (!scheduler || typeof scheduler !== 'object') {
    throw new Error(
      `[kitcn] Context is missing ctx.scheduler for "${pathString}".`
    );
  }
  ...
  if (mode.type === 'after') {
    if (typeof scheduler.runAfter !== 'function') {
      throw new Error(
        `[kitcn] Context is missing ctx.scheduler.runAfter for "${pathString}".`
      );
    }
  ...
  if (typeof scheduler.runAt !== 'function') {
    throw new Error(
      `[kitcn] Context is missing ctx.scheduler.runAt for "${pathString}".`
    );
  }
  return (functionReference: unknown, args: unknown) =>
...
async function executeScheduledProcedure(
  ...
  if (!functionReference) {
    throw new Error(
      `[kitcn] Missing function reference metadata for schedule dispatch: "${pathString}".`
    );
  }
    ...
    async apply(_target, _thisArg, argsList) {
      ...
      if (!entry) {
        if (hasProcedurePrefix(registry, pathString)) {
          throw new Error(
            `[kitcn] Path does not resolve to an action procedure: "${pathString}".`
          );
        }
        throw new Error(`[kitcn] Invalid procedure path: "${pathString}".`);
      }
      ...
      if (getContextType(ctx) !== 'action') {
        throw new Error(
          `[kitcn] Action procedures require action context: "${pathString}".`
        );
      }
      ...
      if (procedure && resolvedType !== procedureType) {
        throw new Error(
          `[kitcn] Procedure type mismatch at "${pathString}". Expected "${procedureType}" but got "${resolvedType ?? 'unknown'}".`
        );
      }
  ...
  return new Proxy(() => {}, {
    ...
    async apply(_target, _thisArg, argsList) {
      ...
      if (!entry) {
        if (hasProcedurePrefix(registry, pathString)) {
          throw new Error(
            `[kitcn] Path does not resolve to a schedulable procedure: "${pathString}".`
          );
        }
        throw new Error(`[kitcn] Invalid procedure path: "${pathString}".`);
      }
      ...
      if (procedureType === 'query') {
        throw new Error(
          `[kitcn] Cannot schedule query procedures: "${pathString}".`
        );
      }
      ...
      if (ctxType !== 'mutation' && ctxType !== 'action') {
        throw new Error(
          `[kitcn] Scheduling requires mutation or action context: "${pathString}".`
        );
      }
...
function createScheduleNamespace(
  ...
  const schedule = {
    ...
    cancel: async (id: ScheduledFunctionId) => {
      ...
      if (!scheduler || typeof scheduler !== 'object') {
        throw new Error(
          '[kitcn] Context is missing ctx.scheduler for "schedule.cancel".'
        );
      }
      if (typeof scheduler.cancel !== 'function') {
        throw new Error(
          '[kitcn] Context is missing ctx.scheduler.cancel for "schedule.cancel".'
        );
      }
