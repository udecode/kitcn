...
describe('server/env', () => {
  // Restore KITCN_CODEGEN after each test so tests are fully isolated.
  afterEach(() => {
  ...
  test('throws at Convex runtime when a required var is missing (no sentinel, no codegenFallback)', () => {
    // Neither KITCN_CODEGEN nor codegenFallback — simulates normal Convex runtime.
    const schema = z.object({
  ...
  test('sentinel alone activates fallback without codegenFallback (CLI default path)', () => {
    // The CLI sets KITCN_CODEGEN=1 but users need not set codegenFallback.
  ...
  test('default runtimeEnv reads optional keys via descriptor fallback when has checks fail', () => {
    const originalEnv = process.env;
    const proxiedEnv = new Proxy(
      {
        BC_REQUIRED_A: 'secret',
