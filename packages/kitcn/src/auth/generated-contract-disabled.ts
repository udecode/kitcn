...
export const getGeneratedAuthDisabledReason = (
  ...
  switch (kind) {
    case 'default_export_unavailable':
      return `Auth runtime is disabled. ${resolvedAuthDefinitionPath} default export is unavailable. Export \`default defineAuth((ctx) => ({ ...options, triggers }))\` and run \`kitcn codegen\`.`;
    case 'missing_auth_file':
      return `Auth runtime is disabled. Create ${resolvedAuthDefinitionPath} with \`export default defineAuth(...)\` and run \`kitcn codegen\`.`;
    case 'missing_default_export':
      return `Auth runtime is disabled. ${resolvedAuthDefinitionPath} exists but does not export a default auth definition. Export \`default defineAuth((ctx) => ({ ...options, triggers }))\` and run \`kitcn codegen\`.`;
  }
};
