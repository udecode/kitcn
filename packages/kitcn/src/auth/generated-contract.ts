...
const withAuthDefaults = <AuthOptions extends BetterAuthOptionsWithoutDatabase>(
  authOptions: AuthOptions
): AuthOptions => {
  const nextRateLimit = {
...
type ProcedureExportLike = {
  ...
  __kitcnRawHandler?: (opts: { ctx: unknown; input: unknown }) => unknown;
};
...
const decorateProcedureExport = (
  ...
  if (typeof procedure.__kitcnRawHandler !== 'function') {
    procedure.__kitcnRawHandler = ({ ctx, input }) =>
