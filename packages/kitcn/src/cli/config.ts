...
const BACKEND_VALUES = new Set<CliBackend>(['convex', 'concave']);

export type BackfillEnabled = 'auto' | 'on' | 'off';
export type CliBackend = 'convex' | 'concave';
export type CliPathsConfig = {
  lib: string;
  shared: string;
  env?: string;
};
...
function parseBackend(
  ...
  if (typeof value === 'string' && BACKEND_VALUES.has(value as CliBackend)) {
    return value as CliBackend;
  }
  throw new Error(
...
export function loadCliConfig(configPathArg?: string): CliConfig {
  const {
