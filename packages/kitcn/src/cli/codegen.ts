...
function findProcedureCallIndex(params: {
  ...
  const exportPattern =
    params.exportName === 'default'
      ...
      : new RegExp(
          `\\bexport\\s+const\\s+${escapeRegex(params.exportName)}\\s*=`,
          'g'
        );
  const exportMatch = exportPattern.exec(params.source);
  if (!exportMatch) {
    return;
  }

  const tail = params.source.slice(exportMatch.index);
  const callPattern = new RegExp(`\\.\\s*${params.type}\\s*\\(`, 'g');
  const callMatch = callPattern.exec(tail);
...
function emitProcedureNameLookupLiteral(lookup: ProcedureNameLookup): string {
...
function emitGeneratedServerPlaceholderFile(functionsDir: string): string {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (procedureNameEntries.length > 0) {
            procedureNameLookup[file] = procedureNameEntries;
          }
        } catch (error) {
