...
const parseArgs = (argv: string[]): AnalyzeOptions => {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === '--hotspot') {
      throw new Error(
        '`--hotspot` was removed. Hotspot is the default mode, so just run `kitcn analyze`.'
      );
    }
    ...
    if (arg === '--entry' || arg.startsWith('--entry=')) {
      throw new Error(
        '`--entry` was removed. Pass the entry regex as the first positional argument, e.g. `kitcn analyze polar.*`.'
      );
    }
    if (
      arg === '--detail-entries' ||
    ...
    if (arg === '--help' || arg === '-h') {
      console.log(`Usage:
  kitcn analyze [entryRegex]
  kitcn analyze --deploy
  kitcn analyze '^convex/functions/auth\\.ts$' --details
...
const shortPath = (inputPath: string): string => {
  ...
  if (inputPath.startsWith('../packages/kitcn/dist/')) {
    return `bcx/${inputPath.slice('../packages/kitcn/dist/'.length)}`;
  }
...
const runHotspotInteractive = async (
  ...
  const renderInteractive = async (): Promise<void> => {
    ...
    console.log(bold('kitcn analyze · interactive'));
