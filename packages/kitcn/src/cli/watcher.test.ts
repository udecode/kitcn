...
describe('cli/watcher', () => {
  ...
  test('startWatcher uses KITCN_CODEGEN_* fallbacks when options are missing', async () => {
    const prevOutputDir = process.env.KITCN_API_OUTPUT_DIR;
    const prevDebug = process.env.KITCN_DEBUG;
    const prevCodegenScope = process.env.KITCN_CODEGEN_SCOPE;
    const prevTrimSegments = process.env.KITCN_CODEGEN_TRIM_SEGMENTS;

    process.env.KITCN_API_OUTPUT_DIR = 'env-out';
    process.env.KITCN_DEBUG = '1';
    process.env.KITCN_CODEGEN_SCOPE = 'orm';
    process.env.KITCN_CODEGEN_TRIM_SEGMENTS = 'plugins,generated,plugins';
    ...
    try {
      ...
      process.env.KITCN_API_OUTPUT_DIR = prevOutputDir;
      process.env.KITCN_DEBUG = prevDebug;
      process.env.KITCN_CODEGEN_SCOPE = prevCodegenScope;
      process.env.KITCN_CODEGEN_TRIM_SEGMENTS = prevTrimSegments;
    }
  });

  test('startWatcher reacts to a real file change with chokidar v5', async () => {
    const tempDir = await mkdtemp(path.join(tmpdir(), 'kitcn-watcher-'));
