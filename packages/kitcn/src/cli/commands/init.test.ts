...
`
  ...
  test('handleInitCommand reinstalls normalized ESLint during Next adoption', async () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'kitcn-init-command-next-eslint-')
    );
    writeShadcnNextApp(tmpDir);
    const packageJsonPath = path.join(tmpDir, 'package.json');
    const packageJson = JSON.parse(
    ...
    packageJson.devDependencies.eslint = '10.10.0';
    fs.writeFileSync(
      packageJsonPath,
      `${JSON.stringify(packageJson, null, 2)}\n`
    );
  ...
  test('handleInitCommand reinstalls ESLint moved to devDependencies', async () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'kitcn-init-command-next-eslint-section-')
    );
    writeShadcnNextApp(tmpDir);
    const packageJsonPath = path.join(tmpDir, 'package.json');
    const packageJson = JSON.parse(
