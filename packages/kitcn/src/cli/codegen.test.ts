    ...
    JSON.stringify(
      {
        ...
        exports: {
          ...
          './orm/aggregate-index': './src/orm/aggregate-index/index.ts',
          './orm/migrations': './src/orm/migrations/index.ts',
...
describe('cli/codegen', () => {
  ...
  test('generateMeta emits server-side procedure name lookup registration', async () => {
    ...
    try {
      ...
      // The lookup is the one parse-derived input, so it lives outside
      // server.ts to keep server.ts emittable before the parse loop.
      ...
      const generatedProcedureNames = fs.readFileSync(
  ...
  test('generateMeta regenerates a generated/server.ts that predates the aggregate capability', async () => {
    const dir = mkTempDir();
    const oldCwd = process.cwd();
    process.chdir(dir);
    ...
    try {
      ...
      writeFile(
        path.join(dir, 'package.json'),
        JSON.stringify({ name: 'stale-server', private: true, type: 'module' })
      );
      writeFile(
        path.join(dir, 'convex', 'schema.ts'),
        `
        import { aggregateIndex, convexTable, defineSchema, integer, text } from 'kitcn/orm';

        export const orders = convexTable(
          'orders',
          { orgId: text().notNull(), amount: integer().notNull() },
          (t) => [aggregateIndex('orders_by_org').on(t.orgId).sum(t.amount)]
        );
      ...
      writeFile(
        path.join(dir, 'convex', '_generated', 'server.js'),
        `
      ...
      // schema with an aggregateIndex, but no capability registered.
      const generatedServerFile = path.join(
        dir,
        'convex',
        'generated',
...
registerProcedureNameLookup({}, "convex");
...
`.trim()
      ...
      writeFile(
        path.join(dir, 'convex', 'orders.ts'),
        `
        import { initCRPC } from './generated/server';

        const c = initCRPC.create();

        export const list = c.query.query(async () => []);
        `.trim()
      );
      ...
      // The repair lands before the parse loop, so the same run still collects
      // procedure metadata rather than degrading to an empty lookup.
      expect(
        fs.readFileSync(
          path.join(dir, 'convex', 'generated', 'procedure-names.gen.ts'),
          'utf-8'
        )
      ).toContain('name: "orders:list"');

      // Idempotent: a second run over the repaired tree changes nothing.
      await expect(generateMeta(undefined, { silent: true })).resolves.toBe(
        undefined
      );
      expect(fs.readFileSync(generatedServerFile, 'utf-8')).toBe(
        generatedServer
      );
    } finally {
      process.chdir(oldCwd);
  ...
  test('generateMeta keeps the procedure name lookup across a scoped run', async () => {
    ...
    try {
      ...
      const procedureNamesFile = path.join(
        dir,
        'convex',
        'generated',
        'procedure-names.gen.ts'
      );
      const afterFullRun = fs.readFileSync(procedureNamesFile, 'utf-8');
      expect(afterFullRun).toContain('"todos:list"');

      // A scoped run never parses modules, so it must not rewrite parse-derived
      // output with the empty lookup it happens to be holding.
  ...
  test('generateMeta leaves generated/server.ts importable after a fatal parse failure', async () => {
    const dir = mkTempDir();
    const oldCwd = process.cwd();
    ...
    try {
      ...
      // Rolling the leaf back would leave an unresolvable import behind in any
      // server.ts the aborted run did not also remove.
      const serverFile = path.join(dir, 'convex', 'generated', 'server.ts');
  ...
  test('generateMeta fails instead of downgrading generated output when the schema cannot load', async () => {
    const dir = mkTempDir();
    const oldCwd = process.cwd();
    ...
    try {
      ...
      writeFile(
        path.join(dir, 'package.json'),
        JSON.stringify({ name: 'schema-boom', private: true, type: 'module' })
      );
      writeFile(
        path.join(dir, 'convex', 'schema.ts'),
        `
        ...
        export const todos = convexTable('todos', { title: text().notNull() });
      ...
      const ormServer = fs.readFileSync(
        path.join(dir, 'convex', 'generated', 'server.ts'),
        'utf-8'
      );

      // A schema that throws must not read as "this project has no ORM": that
      // would rewrite server.ts as the non-ORM variant and delete aggregate.ts.
      writeFile(
        path.join(dir, 'convex', 'schema.ts'),
        `
