...
describe('cli/config', () => {
  test('loads concave.json meta["kitcn"] and ignores kitcn.config.ts', () => {
    ...
    process.chdir(dir);
    try {
      expect(loadCliConfig().paths.shared).toBe('convex/from-json');
    } finally {
  ...
  test('throws for missing explicit config path', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig('./does-not-exist.json')).toThrow(
  ...
  test('throws for explicit kitcn.config.ts path', () => {
    const dir = mkTempDir();
    const oldCwd = process.cwd();
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig('./kitcn.config.ts')).toThrow(
  ...
  test('loads dev.preRun from config', () => {
    ...
    process.chdir(dir);
    try {
      expect(loadCliConfig().dev.preRun).toBe('init');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('loads backend override from config', () => {
    ...
    process.chdir(dir);
    try {
      expect(loadCliConfig().backend).toBe('concave');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws for invalid config values', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid codegen.scope in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws for invalid codegen.trimSegments values', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid codegen.trimSegments in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws for invalid dev.preRun values', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid dev.preRun in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('loads hooks.postAdd from config', () => {
    ...
    process.chdir(dir);
    try {
      expect(loadCliConfig().hooks).toEqual({
        postAdd: ['bun lint:fix'],
      });
    } finally {
      process.chdir(oldCwd);
  ...
  test('throws for invalid hooks.postAdd value', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid hooks.postAdd in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('loads top-level paths config', () => {
    ...
    process.chdir(dir);
    try {
      expect(loadCliConfig().paths).toEqual({
        lib: 'lib-custom',
        shared: 'shared-custom',
        env: 'convex/lib/get-env',
      });
  ...
  test('throws for invalid paths.shared path', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid paths.shared in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws for invalid paths.env path', () => {
    ...
    process.chdir(dir);
    try {
      expect(() => loadCliConfig()).toThrow('Invalid paths.env in');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws when removed top-level outputDir is provided', () => {
    ...
    try {
      expect(() => loadCliConfig()).toThrow('Unknown config key');
      expect(() => loadCliConfig()).toThrow('outputDir');
    } finally {
      process.chdir(oldCwd);
    }
  });
  ...
  test('throws for unknown nested keys', () => {
    ...
    try {
      expect(() => loadCliConfig()).toThrow('Unknown config key');
      expect(() => loadCliConfig()).toThrow('paths.extra');
    } finally {
      process.chdir(oldCwd);
    }
  });
