 ...
 * Source path: packages/convex_helpers/server/zod4.ts
 */
...
> = IsUnknownOrAny<Z> extends true // `unknown` / `any`: we can’t infer a precise return type at compile time
  ? GenericValidator
  : // z.default()
        ...
        : // z.nullable() - handle here to use output types consistently
          ...
          ? ConvexValidatorFromZodOutput<Inner, IsOptional> extends Validator<
            ...
            ? VUnion<
              ...
              >
            : VUnion<
                ConvexValidatorFromZodOutput<Inner, IsOptional>['type'] | null,
                [
                  VRequired<ConvexValidatorFromZodOutput<Inner, IsOptional>>,
                  VNull,
                ],
...
type ConvexValidatorFromZodCommon<
  Z extends zCore.$ZodType,
  IsOptional extends 'required' | 'optional',
> = Z extends Zid<infer TableName> // Basic types
...
type ConvexValidatorFromZodRecord<
  ...
  Value extends zCore.$ZodType,
  IsOptional extends 'required' | 'optional',
> = Key extends // key = v.string() / v.id() / v.union(v.id())
  | zCore.$ZodString
  | Zid<any>
...
type BrandIfBranded<
  InnerType,
  Validator extends zCore.SomeType,
