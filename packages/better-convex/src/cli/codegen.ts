...
function emitGeneratedLookupHelper(): string {
  return `function getGeneratedValue(root: unknown, path: readonly string[]): unknown {
  let current = root;
...
function emitGeneratedProcedureFunctionReferenceHelper(): string {
  return `type ProcedureArgsFromExport<TExport> =
...
function emitGeneratedServerFile(
  ...
  hasManualTriggersExport: boolean,
  ...
  const hasOrmSchema = hasRelationsMetadata || hasRelationsExport;
...
`;
  ...
  const namedSchemaImports = [
    useRelationsExport ? 'relations' : null,
    hasManualTriggersExport && triggersImportSource === 'schema'
      ? 'triggers'
      : null,
  ].filter((value): value is string => value !== null);
  const schemaImportSpecifiers =
    defaultSchemaImport && namedSchemaImports.length > 0
      ? `${defaultSchemaImport}, { ${namedSchemaImports.join(', ')} }`
      : defaultSchemaImport || `{ ${namedSchemaImports.join(', ')} }`;
  const baseSchemaIdentifier = useRelationsExport ? 'relations' : 'schema';
  const triggersImportLine =
    hasManualTriggersExport && triggersImportSource === 'triggers'
      ? `import { triggers } from ${triggersImportLiteral};\n`
      : '';
  const attachTriggersHelper = hasManualTriggersExport
    ? `
const attachGeneratedTriggers = <TSchema extends object, TTriggers>(
  schema: TSchema,
  schemaTriggers: TTriggers
) => {
  ...
  return schema as TSchema & { [OrmSchemaTriggers]: TTriggers };
};
`
    : '';
  const ormSchemaDeclaration = hasManualTriggersExport
    ? `const ormSchema = attachGeneratedTriggers(${baseSchemaIdentifier}, triggers);`
    : `const ormSchema = ${baseSchemaIdentifier};`;
...
import { httpAction, internalMutation } from ${serverTypesImportLiteral};
import ${schemaImportSpecifiers} from ${schemaImportLiteral};
${triggersImportLine}
${migrationsImportLine}
${emitGeneratedLookupHelper()}
...
function emitGeneratedMigrationsFile(
  ...
  const schemaImportPath = getSchemaImportPath(outputFile, functionsDir);
  const schemaImportLiteral = asSingleQuotedImport(schemaImportPath);
  const migrationSchemaImport =
    hasRelationsExport && !hasRelationsMetadata ? '{ relations }' : 'schema';
  const migrationSchemaType =
    hasRelationsExport && !hasRelationsMetadata
      ? 'typeof relations'
      : 'typeof schema';
...
${emitGeneratedLookupHelper()}
${emitGeneratedProcedureFunctionReferenceHelper()}
...
function emitApiObject(
  ...
  for (const key of mergedKeys) {
    ...
    const runtimeAccess = `getGeneratedValue(convexApi, ${JSON.stringify([
      ...pathSegments,
      key,
    ])})`;
...
function emitProcedureRegistryEntries(
  ...
  return entries
    .map((entry) => {
      ...
      const functionRefAccess = `getGeneratedFunctionReference<${JSON.stringify(
        entry.type
      )}, typeof import(${JSON.stringify(moduleImportPath)}).${
        entry.exportName
      }>(${entry.internal ? 'internal' : 'api'}, ${JSON.stringify([
...
export async function generateMeta(
  ...
  if (debug) {
    if (generateApi) {
      ...
      logger.info(`Running better-convex codegen (mode=${modeLabel})...\n`);
    }
  }
  ...
  const hasTriggersMetadata = schemaMetadata.hasTriggers;
  ...
  const hasOrmSchema = hasRelationsMetadata || hasRelationsExport;
  if (hasManualTriggersExport && !hasOrmSchema) {
    throw new Error(
