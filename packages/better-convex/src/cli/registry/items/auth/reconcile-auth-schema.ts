...
const BETTER_CONVEX_CODEGEN_GLOBAL_KEY = '__BETTER_CONVEX_CODEGEN__';
...
const withAuthSchemaEnv = async <T>(run: () => Promise<T>): Promise<T> => {
  ...
  const originalCodegenSentinel = globalScope[BETTER_CONVEX_CODEGEN_GLOBAL_KEY];
  ...
  globalScope[BETTER_CONVEX_CODEGEN_GLOBAL_KEY] = true;

  try {
    return await run();
  } finally {
    if (typeof originalCodegenSentinel === 'undefined') {
      delete globalScope[BETTER_CONVEX_CODEGEN_GLOBAL_KEY];
    } else {
      globalScope[BETTER_CONVEX_CODEGEN_GLOBAL_KEY] = originalCodegenSentinel;
    }
...
export const renderManagedAuthSchemaFile = async ({
  ...
  if (kind === 'extension') {
    const result = await createSchemaExtensionOrm({
      ...
      file: outputPath,
      regenerateCommand: 'npx better-convex add auth --yes',
      tables,
    });
    return result.code;
  }
  ...
  const result = await createSchema({
    ...
    file: outputPath,
    regenerateCommand: 'npx better-convex add auth --preset convex --yes',
    tables,
  });
  return result.code;
};
...
export const loadAuthOptionsFromDefinition = async (
  ...
  return withAuthSchemaEnv(async () => {
    ...
    const tempAuthPath = path.join(
      authDir,
      `.tmp-better-convex-auth-${tempId}.ts`
    );
    const tempGeneratedAuthBase = `.tmp-better-convex-define-auth-${tempId}`;
