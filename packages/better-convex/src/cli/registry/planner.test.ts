...
describe('cli registry planner', () => {
  ...
  test('reconciles scaffold files before turning them into plan files', async () => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'better-convex-registry-reconcile-')
    );
    ...
    try {
      ...
      const plan = await buildPluginInstallPlan({
        ...
        descriptor: {
          ...
          keywords: [],
          label: 'Fake',
          packageName: '@better-convex/fake',
          presets: [
            {
              description: 'default',
              key: 'default',
  ...
  test('adds a live bootstrap operation when the plugin requires local post-codegen bootstrap', async () => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'better-convex-registry-live-bootstrap-')
    );
    const originalCwd = process.cwd();
    process.chdir(dir);
    ...
    try {
      ...
      expect(plan.operations).toContainEqual({
        ...
        command: 'better-convex dev --bootstrap',
      });
    } finally {
      process.chdir(originalCwd);
    }
