...
export const VERIFY_HELP_TEXT = `Usage: better-convex verify [options]

Run a local runtime proof through the Better Convex dev pipeline.
...
function assertNoVerifyLifecycleFlags(args: string[]) {
  if (args.includes('--once') || args.includes('--bootstrap')) {
    throw new Error(
      '`better-convex verify` already runs one-shot runtime proof. Do not pass `--once` or `--bootstrap`.'
    );
  }
}
...
export const handleVerifyCommand = async (
  ...
  const { loadBetterConvexConfig } = resolveRunDeps(deps);
  ...
  if (backend !== 'convex') {
    throw new Error(
      '`better-convex verify` is only supported for backend convex.'
    );
  }
  ...
  if (getAggregateBackfillDeploymentKey(targetArgs) !== 'local') {
    throw new Error(
      '`better-convex verify` is local-only. Remove remote deployment flags like `--prod`, `--preview-name`, and `--deployment-name`.'
    );
  }
