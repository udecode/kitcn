...
export const makeBatch = privateMutation.use(resend.middleware())
  ...
  .mutation(async ({ ctx }) => {
    const caller = createResendCaller(ctx);
    await ctx.orm.delete(resendStorageTables.resendNextBatchRun).allowFullScan();
...
export const getAllContentByIds = privateQuery.use(resend.middleware())
  ...
  .query(async ({ ctx, input }) => {
    ...
    const rows = await ctx.orm.query.resendContent.findMany({
      where: inArray(resendStorageTables.resendContent.id, input.contentIds),
    ...
    const entries = rows
      .filter(
        (row): row is { id: string; content: ArrayBuffer } =>
...
export const getEmailsByIds = privateQuery.use(resend.middleware())
  ...
  .query(async ({ ctx, input }) => {
    ...
    const rows = await ctx.orm.query.resendEmails.findMany({
      where: inArray(resendStorageTables.resendEmails.id, input.emailIds),
