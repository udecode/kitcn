/**
 * ConvexQueryClient - Real-time subscription bridge for TanStack Query + Convex (Solid.js)
 *
 * ## Why This Exists
 *
...
/**
 * Auth store interface for checking auth state.
 * Framework-agnostic store that provides auth information.
 */
export type AuthStore = {
  get(key: 'isAuthenticated' | 'isLoading'): boolean;
  get(key: 'onQueryUnauthorized'): (info: { queryName: string }) => void;
  get(key: 'isUnauthorized'): (error: unknown) => boolean;
  get(key: 'expiresAt' | 'token'): string | null;
};
...
export interface ConvexQueryClientOptions {
   ...
   * Prevents wasteful unsubscribe/subscribe cycles from quick
   * back/forward navigation. Set to 0 to unsubscribe immediately.
   * @default 3000
   */
...
/**
 * Bridges TanStack Query with Convex real-time subscriptions (Solid.js version).
 *
 * Uses push-based `ConvexClient.onUpdate` instead of pull-based `watchQuery`.
 *
 * ## Setup
 *
 * ```ts
 ...
 *    ConvexQueryClient creates a WebSocket subscription via `onUpdate`.
 *
...
export class ConvexQueryClient {
   ...
   * Used to debounce unsubscribe/subscribe cycles from quick navigation.
   */
   ...
   * Regular queries already handle this via enabled: false in hooks.
   */
  ...
  /**
   * Handle a subscription error for a specific query.
   * Pushes error state to TanStack cache and handles auth-related errors.
   */
  ...
  constructor(
    ...
    if (isServer) {
      this.serverHttpClient = new ConvexHttpClient(
        typeof client === 'string' ? client : (client as any).address,
        {
  ...
  /**
   * Create a Convex subscription for a query and register it.
   * Uses push-based `ConvexClient.onUpdate` callback pattern.
   */
    ...
    const unsub = this.convexClient.onUpdate(
      ...
      (result: unknown) => {
        notifyManager.batch(() => {
          ...
          const hasResultValue = result !== null && result !== undefined;
          ...
          if (hasResultValue || !hasExistingData) {
            this.queryClient.setQueryData(
              queryKey,
              this.transformer.output.deserialize(result)
            );
          }
        });
      },
    ...
    return queryCache.subscribe((event) => {
      ...
      switch (event.type) {
        ...
        case 'added': {
          ...
          this.createSubscription(event.query.queryHash, event.query.queryKey);
          break;
        }

        // Create subscription when first observer is added (query enabled)
        case 'observerAdded': {
          // Cancel any pending unsubscribe (handles remount scenarios)
          this.cancelPendingUnsubscribe(event.query.queryHash);
        ...
        // Grace period prevents wasteful unsubscribe/subscribe from quick navigation
        // Cache data persists for gcTime (default 5 min) for instant back-navigation
