...
import type { ExtractPaginatedItem } from '../crpc/types';
...
/** Return type for infinite query hooks - Solid version */
export type CreateInfiniteQueryResult<T> = {
  data: T[];
  pages: T[][];
...
type StaleCursorRecoveryOptions = {
...
/**
 * Stale cursor recovery logic for Solid.
 *
...
function runStaleCursorRecovery({
  ...
  if (combined.isFetchNextPageError) {
    ...
    if (hasPage0Data && hasSubsequentErrors && page0Data?.continueCursor) {
      ...
      if (state.autoRecoveryAttempted !== recoveryKey) {
...
/**
 * Internal infinite query using TanStack Query + convexQuery (Solid version).
 * Each page gets:
 * - Convex WebSocket subscription (real-time reactivity)
 * - TanStack Query retry on timeout errors
 *
 * Use `createInfiniteConvexQuery` for the public API with auth handling.
 */
function createInfiniteQueryInternal<Query extends PaginatedQueryReference>(
  query: Query,
  args: PaginatedQueryArgs<Query>,
  options: InfiniteQueryOptions<PaginatedQueryItem<Query>>
): CreateInfiniteQueryResult<PaginatedQueryItem<Query>> {
  ...
  const isAuthLoading = () => store.isLoading;
  const meta = useMeta();
  ...
  // Helper to get/set pagination state from queryClient
  ...
  // State: tracks pages with cursors
  ...
  const tanstackQueries = createMemo(() =>
    state().pageKeys.map((key, index) => {
      ...
      return {
        ...
        enabled: !skip() && !!currentState.queries[key],
  ...
  const combined = createQueries(() => ({
    ...
    combine: (results: any[]) => {
      ...
      // Get first result for base fields
  ...
  // Return a reactive object (getters for reactivity in Solid)
  return {
    get data() {
      return combined.data as PaginatedQueryItem<Query>[];
    },
    get pages() {
      return combined.pages as PaginatedQueryItem<Query>[][];
    },
    get failureReason() {
    ...
    get isLoading() {
      return combined.status === 'LoadingFirstPage';
    },
    get dataUpdatedAt() {
      return combined.dataUpdatedAt;
    },
    get status() {
      return combined.status as PaginationStatus;
    },
    get hasNextPage() {
      return hasNextPage();
    },
...
/**
 * Infinite query using cRPC-style options (Solid version).
 * Accepts options from `crpc.posts.list.infiniteQueryOptions()`.
 *
 * @example
 * ```tsx
 * const crpc = useCRPC();
 * const result = createInfiniteConvexQuery(
 *   crpc.posts.list.infiniteQueryOptions({ userId }, { limit: 20 })
 * );
 * ```
 */
export function createInfiniteConvexQuery<
...
>(
...
): CreateInfiniteQueryResult<TItem> {
  ...
  const { queryName, args, limit, authType, skipUnauth } = optionsMeta;
  ...
  const isUnauthorized = () =>
    authType === 'required' && !store.isLoading && !store.isAuthenticated;

  // Determine if we should skip the query
  const shouldSkip = () =>
    factoryEnabled === false ||
    (authType === 'required' && store.isLoading) ||
    (authType === 'required' && !store.isAuthenticated);
  ...
  createEffect(() => {
    if (isUnauthorized() && !skipUnauthFinal) {
      store.onQueryUnauthorized({ queryName });
    }
  });
  ...
  return {
    ...
    get isError() {
      return !!authError() || result.isError;
    },
    get isPlaceholderData() {
    ...
    get isLoading() {
      ...
      const ae = authError();
      return (
        (authLoadingApplies && store.isLoading) ||
