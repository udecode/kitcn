...
1. `@better-convex/<plugin>`: runtime capability, middleware, stable helpers, shared types.
2. `packages/better-convex/src/cli/plugins/<plugin>/...`: scaffold templates for local schema/runtime files.
...
Plugin packages should define their runtime entry with `definePlugin('<key>', ...)` from `better-convex/plugins` and expose a named plugin value such as `ResendPlugin`.
...
11. Plugin runtime code must use ORM context (`ctx.orm`) when it touches Better Convex schema.
