...
| `DirectAggregate` | Not available | Use `better-convex/aggregate` entrypoint if needed |
...
`better-convex deploy` runs `aggregateBackfill` in `resume` mode for both metric and rank indexes, then waits for all to report `READY`.
...
| Feature | Workaround |
|---|---|
| `DirectAggregate` (table-independent) | Use `better-convex/aggregate` entrypoint |
...
| `clear` / `clearAll` | `better-convex aggregate rebuild` |
