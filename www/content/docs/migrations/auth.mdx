...
| Auth Contract | `createAuthOptions/getAuth` split | `default defineAuth((ctx) => ({ ...options, triggers }))` |
...
<Tabs groupId="auth-migration-path" items={["CLI", "Manual"]} persist>
  <Tab value="CLI">
    ...
    The generated `auth.config.ts` starts with `getAuthConfigProvider()`. That
    first pass is intentional. It lets the initial bootstrap complete before
    `JWKS` exists. Once that bootstrap is done, switch to the static `JWKS`
    version shown in the Manual tab's `auth.config.ts` step.

    ### Done

    That's it. The CLI path already updates the auth contract, HTTP routes,
    client wiring, provider wiring, and schema integration for you.
  </Tab>
  <Tab value="Manual">
...
| Before | After | Why |
|--------|-------|-----|
| `createAuthOptions`/`getAuth` split | Single `defineAuth((ctx) => ({ ... }))` | One contract, less boilerplate |
| Runtime exports from `auth.ts` | Generated runtime in `convex/functions/generated.ts` | Codegen handles wiring for you |
...
<Tabs groupId="schema-style" items={["ORM", "DB"]} persist>
  <Tab value="ORM">
    ...
    ```bash
    npx @better-auth/cli generate -y --output convex/functions/authSchema.ts --config convex/functions/generated/auth.ts
    ```

    Merge generated auth tables into your ORM schema:
    ...
    export const tables = {
      ...authSchema,
      // ...your app convexTable definitions
    };
    ...
    <Callout icon={<InfoIcon />}>
    CLI emits ORM-flavored output (`convexTable`, `defineSchema`) when ORM schema metadata is detected.
    </Callout>
  </Tab>
  <Tab value="DB">
    Generate auth tables:
    ...
    Merge generated auth tables into your Convex schema:
    ...
    export default defineSchema({
      ...authSchema,
      // ...your app defineTable definitions
    });
    ```
  </Tab>
</Tabs>

For fully manual table definitions, see [Auth Server Setup](/docs/auth/server#4-update-schema).
...
<Compare>
  ...
  <CompareItem title="After">
    ...
    const headers = await getHeaders(ctx);
    const auth = getAuth(ctx); // generated getAuth from convex/functions/generated.ts
    const session = await auth.api.getSession({ headers });
    ```
  </CompareItem>
</Compare>
...
- [ ] **Generated runtime:** Run `npx better-convex dev` so `convex/functions/generated.ts` exists
- [ ] **auth.ts:** Rewrite to `export default defineAuth((ctx) => ({ ...options, triggers }))`
...
- [ ] **Env vars:** Run `npx better-convex env push --auth` for JWKS
...
Generated runtime can be disabled when `convex/functions/auth.ts` is missing or lacks a default export.
...
Use generated config entrypoint (not `auth.ts`):

```bash
npx @better-auth/cli generate -y --output convex/functions/authSchema.ts --config convex/functions/generated/auth.ts
```

If it still fails, re-run `better-convex codegen` first so generated exports are up to date.
