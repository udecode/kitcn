...
description: Migrate from @convex-dev/better-auth to kitcn.
links:
...
<Accordions type="single">
  <Accordion id="production-only-migrate-component-data-to-app-namespace" title="Production only: Migrate component data to app namespace">
    ...
    **2) Download migration utility files**
    ...
    curl -fsSL https://raw.githubusercontent.com/udecode/kitcn/main/tooling/migrations/better-auth-component/cli.ts -o ./better-auth-component-migration/cli.ts
    curl -fsSL https://raw.githubusercontent.com/udecode/kitcn/main/tooling/migrations/better-auth-component/id-v6.ts -o ./better-auth-component-migration/id-v6.ts
    curl -fsSL https://raw.githubusercontent.com/udecode/kitcn/main/tooling/migrations/better-auth-component/migrate-snapshot.ts -o ./better-auth-component-migration/migrate-snapshot.ts
    curl -fsSL https://raw.githubusercontent.com/udecode/kitcn/main/tooling/migrations/better-auth-component/zip.ts -o ./better-auth-component-migration/zip.ts
    ```
...
<Tabs groupId="auth-migration-path" items={["CLI", "Manual"]} persist>
  <Tab value="CLI">
    ...
    pulling in the wider kitcn scaffold.
  </Tab>
  <Tab value="Manual">
...
For TanStack Start, follow Steps 1-3 above for the core auth migration. This
section only covers the framework-specific wiring.

<Callout icon={<InfoIcon />}>
TanStack Start uses the default kitcn auth flow: run `npx kitcn add auth --yes`
once, use `npx kitcn add auth --schema --yes` after auth table changes in
`<functionsDir>/auth.ts`, and keep `npx kitcn dev` as the local Convex + codegen
loop. There is no separate `auth:generate` step on this path.
</Callout>

### Server Setup

Create server-side auth helpers using `kitcn/auth/start`:
...
export const Route = createRootRouteWithContext<{
  ...
  beforeLoad: async (ctx) => {
    return await syncConvexAuthForStartLoader({
      convex: ctx.context.convexQueryClient,
      getToken: getAuth,
    });
  },
