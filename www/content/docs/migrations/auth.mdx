...
| Trigger signature `(ctx, ...)` | Trigger signatures `(...)` with closure `ctx` | Cleaner API — `ctx` comes from the `defineAuth` closure |
...
<Compare>
  ...
  <CompareItem title="After (lib/convex/auth-client.ts)">
    ...
    import { convexClient } from "better-auth/client/plugins";
    import { createAuthClient } from "better-auth/react";
