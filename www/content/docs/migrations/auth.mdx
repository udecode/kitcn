...
<Compare>
  ...
  <CompareItem title="After (direct pattern)">
    ...
    Runtime handlers (`getAuth`, CRUD/JWKS handlers, trigger handlers, static `auth`) are generated in `convex/functions/generated.ts` after `better-convex dev` / `better-convex codegen`.
  </CompareItem>
</Compare>
...
| Before | After |
|--------|-------|
| `createAuthOptions/getAuth` split | `default defineAuth((ctx) => ({ ...options, triggers }))` |
| Runtime exports from `auth.ts` | Generated runtime in `convex/functions/generated.ts` |
| `authComponent.adapter(ctx)` | Generated `getAuth(ctx)` with context-aware adapter |
| Trigger signature `(ctx, ...)` | Doc-first trigger signatures `(...)` with closure `ctx` |
| Component namespace routing | Generated contract + `internal.generated.*` handlers |

<Callout icon={<InfoIcon />}>
**Context-aware adapter:** use generated `getAuth(ctx)`; generated runtime picks direct DB access in queries/mutations and HTTP adapter behavior in actions/HTTP contexts.
</Callout>
...
<Tabs groupId="schema-style" items={["ORM", "DB"]} persist>
  <Tab value="ORM">
    Use this when your app schema uses `better-convex/orm` (`convexTable`, `defineSchema`).
  ...
  <Tab value="DB">
    Use this when your app schema uses `convex/server` (`defineTable`, `defineSchema`).
    ...
    <Callout icon={<InfoIcon />}>
    Without ORM metadata, CLI output stays on Convex `defineTable` format.
    </Callout>
  </Tab>
</Tabs>

For fully manual table definitions, see [Auth Server Setup](/docs/auth/server#3-update-schema).
...
### `TypeError: e.auth is not a function` on `/api/auth/*`

Your HTTP routes are likely wiring old auth imports.

Use generated `getAuth`:
...
and keep either:
1. `app.use(authMiddleware(getAuth))`, or
2. `registerRoutes(http, getAuth, ...)`.

### "Auth triggers not firing"

Auth triggers must be defined inside `defineAuth((ctx) => ({ triggers: ... }))`.

They only fire through Better Auth flows, not direct database operations.

### `MessageChannel is not defined`

Add the HTTP polyfill from Step 5 if your runtime lacks `MessageChannel`.

### Auth routes missing from generated API types

In `convex/functions/http.ts`, export your router as:

```ts
export const httpRouter = router({ ... });
```

This name is required for codegen to include HTTP route types.
