...
- Define relations on the default schema export with `.relations(...)`.
...
import { convexTable, defineSchema, text, boolean, integer, id, index, uniqueIndex } from 'better-convex/orm';
...
`convex/functions/generated/server.ts` exports `initCRPC` pre-wired with ORM context:
...
<Compare>
  ...
  <CompareItem title="After (ORM + cRPC)">
    ...
    import { createPostsCaller } from './generated/posts.runtime';
    ...
    export const similarPosts = publicAction
      ...
      .action(async ({ ctx, input }) => {
        ...
        const caller = createPostsCaller(ctx);
        const rows = await caller.fetchByIds({
          ids: hits.map((h) => h.id),
        });
