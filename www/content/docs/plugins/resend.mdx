...
`better-convex add resend` writes this for you. Customize resend tables and relations by editing `convex/lib/plugins/resend/schema.ts` directly.

If `paths.env` is missing, `better-convex add resend` also creates `convex/lib/get-env.ts`, writes `paths.env` into `concave.json`, and adds:
