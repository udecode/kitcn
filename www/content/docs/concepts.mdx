...
// Aggregate runtime for advanced/ranked metrics
const topUsers = await userAggregate.rank(ctx, { limit: 10 });
```

_Use `db.query.*.count(...)` and `db.query.*.aggregate(...)` for scalar metrics. Use `TableAggregate` for ranked and namespace-ordered metrics._
