...
5. Env: `npx kitcn env push` for `--prod`, `--rotate`, or explicit repair. During local dev, `convex/.env` edits auto-sync.
6. Local auth assumes `http://localhost:3000`. If you change the port, update `SITE_URL` in `convex/.env` and the matching client env var (`NEXT_PUBLIC_SITE_URL` for Next.js, `VITE_SITE_URL` for Start/Vite).
