...
**CLI bootstrap:**
1. Run `better-convex init` in an existing app, or `better-convex create -t next --yes` for a fresh scaffold
...
3. On backend `convex`, `create`, `init`, and `dev` run `convex init` before Better Convex codegen.
4. For auth bootstrap, use `better-convex dev --once --typecheck disable`, then `better-convex env push --auth`.
...
3. Use [Templates](/docs/templates) only as snippet reference
...
| [CLI Registry](/docs/cli/registry) | Create starters with `better-convex create`, adopt apps with `better-convex init`, add features with `better-convex add`, inspect plans with `view` and `info`. |
| [Templates](/docs/templates) | Snippet reference for full-file patterns. Use when you want to study or copy a specific implementation. |
...
2. [CLI Registry](/docs/cli/registry) → run `better-convex create` / `better-convex init` / `better-convex add`
