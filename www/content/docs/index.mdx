...
**CLI bootstrap:**
1. Run `better-convex init` in an existing app, or `better-convex init -t next --yes` for the full curated Next.js scaffold
2. Add only the features you chose with `better-convex add <plugin>`
3. Init already generates the first Better Convex runtime pass. Run `better-convex dev` or `better-convex codegen` after later schema/procedure changes.
4. For explicit non-interactive project-backed bootstrap, pass `--team <team_slug> --project <project_slug> --dev-deployment local` to `better-convex init`.
5. If browser smoke says use `http://localhost:3000` but port 3000 is occupied, run smoke on the actual dev server port instead and note it in the report.
...
| Stack | Key Docs |
|-------|----------|
| Next.js + Better Auth | [CLI](/docs/cli), [Next.js Setup](/docs/nextjs), [Auth Server](/docs/auth/server) |
...
| [CLI](/docs/cli) | Bootstrap with `better-convex init`, add features with `better-convex add`, inspect plans with `view` and `info`. |
...
2. [CLI](/docs/cli) → run `better-convex init` / `better-convex add`
