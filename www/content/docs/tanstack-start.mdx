...
**Note:** On npm, the first `npx` run may prompt to install `kitcn@latest`. If
your npm install stops on a peer-resolution error, rerun the same command with
`npm_config_legacy_peer_deps=true`, or use the `pnpm` or `bun` tab instead.
</Callout>
