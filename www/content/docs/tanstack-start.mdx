...
`init -t start` creates the shadcn TanStack Start shell and layers in the kitcn baseline — backend files, client providers, local env scaffolding, and a starter messages demo.
...
export const Route = createRootRouteWithContext<{
  ...
  beforeLoad: async ({ context }) => {
    const auth = await syncConvexAuthForStartLoader({
...
export const requireAuth = (context: { isAuthenticated: boolean }) => {
...
`syncConvexAuthForStartLoader` accepts either a `ConvexReactClient` or the
`ConvexQueryClient` from router context. It updates the browser Convex client,
updates the server HTTP client during SSR, skips duplicate `setAuth` calls for
the same token, and clears auth when no token is present.
