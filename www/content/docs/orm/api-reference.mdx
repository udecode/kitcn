...
```ts showLineNumbers
import { index, uniqueIndex } from 'kitcn/orm';
...
```ts showLineNumbers
import { check, convexTable, gt, integer } from 'kitcn/orm';
...
```ts showLineNumbers
import { searchIndex } from 'kitcn/orm';
...
```ts showLineNumbers
import { vector, vectorIndex } from 'kitcn/orm';
...
objectOf(unionOf(text().notNull(), integer().notNull()).notNull())
```
...
The generated module wires `schema`, `triggers`, `ormFunctions`, `capabilities`, and `internalMutation`, then exports `scheduledMutationBatch` and `scheduledDelete`. See the generated file for the full setup.
</Callout>

**`capabilities`**

Optional ORM subsystems. Convex bundles everything a module statically imports and has no dynamic
`import()`, so `kitcn/orm` does not reach them on its own. `kitcn codegen` registers the ones the
app uses; a hand-written `createOrm()` passes them itself:
...
| Capability | Registered when | Required by |
|---|---|---|
| `aggregateCapability()` | the schema declares an `aggregateIndex` or `rankIndex` | `aggregateIndex`/`rankIndex` schemas, filtered `count()`, `count({ select: { field } })`, `aggregate()`, `groupBy()`, `rank()`, relation `_count`, and `orm.api()` backfill handlers |
| `migrationCapability()` | `<functionsDir>/migrations/manifest.ts` exists | `orm.api()` migration handlers |

Every procedure module imports `generated/server.ts`, so `kitcn codegen` registers a capability only
when the app uses it — otherwise the runtime would land in every Convex function of the app.

Unfiltered `count()` and `aggregate({ _count: true })` are served by the native Convex count syscall
...
- `maxScan`: `number` (only valid with cursor pagination; caps scan-fallback paths like predicate `where` and post-filter-only plans)
...
Children are read through an index that leads with the relation's foreign key. A stage `where` rides that index when one extends it — `where: { numLikes: { gt: 10 } }` on `posts` reads the `by_author_likes` (authorId, numLikes) range instead of every post by the author. Children then arrive in that index's order, so a lowered range field orders them ahead of creation time. Declare an index whose fields are `(...foreign key, ...filtered fields)` for the stage filters you page on.
...
<Cards>
  ...
  <Card title="RLS" href="/docs/orm/rls" />
</Cards>
