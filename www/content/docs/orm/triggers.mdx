...
When aggregates are wrapped with `createAggregate(...)` from `better-convex/aggregate`, you can pass `aggregate.trigger` directly or call `aggregate.trigger(change, ctx)`.
