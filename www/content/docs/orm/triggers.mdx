...
Compose aggregate handlers inside `change`:

With `TableAggregate` from `better-convex/aggregate`, you can pass `aggregate.trigger` directly or call `aggregate.trigger(change, ctx)`.

```ts title="convex/functions/schema.ts"
export const triggers = defineTriggers(relations, {
  todos: {
    change: aggregateTodosByUser.trigger,
  },
  todoComments: {
    change: async (change, ctx) => {
...
No array API is needed; compose effects directly in one function.
...
2. Keep expensive work in `scheduler.runAfter`.
...
4. `ctx.innerDb` bypasses recursive dispatch.
