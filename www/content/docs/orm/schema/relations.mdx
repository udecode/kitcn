...
Start with a non-strict pattern when you only need relation loading parity.
Add strict constraints only when you need database-level guarantees.

#### Step 1: Non-Strict (Parity-First)
...
const relations = defineRelations({ posts, videos, comments }, (r) => ({
  posts: {
    comments: r.many.comments({
      from: r.posts.id,
      to: r.comments.postId,
...
// posts, videos unchanged from Step 1

const comments = convexTable(
  'comments',
  {
    body: text().notNull(),
    targetType: text().notNull(), // 'post' | 'video'
...
<Callout icon={<InfoIcon />}>
Drizzle parity includes polymorphic-style relation filtering via relation `where`.
True polymorphic FK integrity is app/schema-level (Step 2), not a single built-in polymorphic FK primitive.
</Callout>

#### Typed Query Synthesis (`polymorphic`)

Once `comments.post` / `comments.video` `one()` relations exist, you can synthesize a typed discriminated union field in query results:

```ts showLineNumbers
import { z } from 'zod';

const commentTargetSchema = z.discriminatedUnion('targetType', [
  z.object({
...
`polymorphic` is available on `findMany`, `findFirst`, and `findFirstOrThrow`. It cannot be used together with `pipeline`.
