...
- `where` filters the rows the source's range produces, after the read.
...
Anchoring each source is what keeps the read bounded. A shared `.withIndex(...)` plus a per-source `where` makes every source walk the same range and drop the misses after reading them.
