...
Up to 64 ranges are read as one merged stream, so `orderBy` still sorts across the whole result, not per value. A wider union is read one range after another instead, which keeps it index-bounded at any list length but can only produce the index's own order.
...
- An `orderBy` that sorts across values, such as `createdAt`, on a union wider than 64 ranges. Interleaving values needs all their ranges open at once, which is the fan-out that cap refuses.
