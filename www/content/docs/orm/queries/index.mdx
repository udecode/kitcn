...
```ts showLineNumbers title="convex/queries.ts"
...
For the complete operator list, see [API Reference](/docs/orm/api-reference).

### String Operators

The ORM supports familiar string matching operators. Most run post-fetch, but a few can leverage indexes. For the full operator list, see [API Reference](/docs/orm/api-reference).
...
These operators are post-fetch. In the typed API, they currently require explicit `.withIndex(...)` so scan scope is deliberate. For the full list with scalable workarounds for each, see [API Reference](/docs/orm/api-reference).
...
For cursor queries that fall back to a scan (predicate `where` or post-filter-only plans), use `maxScan`.
...
| Slow ordering | Put the primary `orderBy` field in an index, immediately after the fields the `where` pins |
