...
In this guide, we'll walk through built-in data migrations for kitcn. You'll learn when to use them, how to author migration files, and how they integrate with `kitcn dev` and `kitcn deploy` for safe, auditable schema evolution.
...
npx kitcn migrate create backfill_todo_priority
```
...
`down` is optional and always explicit. `kitcn dev` and `kitcn deploy` only auto-run `up` migrations. If you try to roll back a migration without a `down` handler, the CLI will error.
...
npx kitcn migrate down --to 20260227_080239_backfill_todo_priority --prod
```
...
`kitcn deploy` orchestrates the full lifecycle automatically:
...
```json title="kitcn.json" showLineNumbers {3-11}
{
  "deploy": {
    "migrations": {
      "enabled": "auto",
...
`kitcn dev` also supports auto-migration with more relaxed defaults (`strict: false`, `allowDrift: true`). See [CLI Backend](/docs/cli/backend) for the full configuration reference.
...
`generated/server:migrationStatus` is an internal **query**, so reading status
never takes a write transaction and can back a live subscription.

| Arg | Description |
|-----|-------------|
| `limit` | Number of most recent runs to list. Defaults to `25`, capped at `MAX_STATUS_RUN_LIMIT` (`100`). Higher values are clamped to the cap. |
| `runId` | Return only this run. Ignores `limit`. |

`activeRun` is always resolved independently of `runId`, so a status call
filtered to one run still reports whether any run is in flight. The
`migrations`, `pending`, and `drift` fields always cover every authored
migration and are not affected by `limit`.

Import the cap when you need to assert against it:
