...
| `groupBy({ by, _count/_sum/_avg/_min/_max })` | No | Yes | Finite-constrained `by` fields only; no `having`/`orderBy`/`skip`/`take` in v1 |
| Window analytics (`having` / window functions) | No | No | Deferred |
| Aggregate runtime (`better-convex/aggregate`) | Yes | Yes | Built-in `TableAggregate` for ranked/namespace use cases (no `@convex-dev/aggregate` dependency) |
...
| Trigger integration with aggregate runtime | Manual | Yes | Compose `TableAggregate.trigger(change, ctx)` |
