...
**Same API surface:**
- Table builders and relation primitives (`convexTable`, `.relations(...)`, `one`, `many`)
...
- [ ] Define relations on the default schema export with `.relations(...)` and load them with `with`
...
| `defineRelations()` | `defineSchema(...).relations(...)` | Same relation builder, attached to the default schema export |
...
| Capability | Drizzle | kitcn ORM | Notes |
|---|---|---|---|
...
- SQL database migrations and database-side constraints/triggers (use `kitcn migrate ...` for online data migrations)
- Custom SQL data types
...
| Issue | Cause | Fix |
|---|---|---|
| **`ctx.orm` is missing** | ORM not attached to context | Run `kitcn codegen` to generate `convex/functions/generated/server.ts` |
...
```ts showLineNumbers
import { getTableColumns, getTableConfig } from 'kitcn/orm';
