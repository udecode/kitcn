...
| `groupBy` in query builder | `db.query.*.groupBy({...})` | Prisma-style `by` + `_count/_sum/_avg/_min/_max`; requires finite `where` constraints on `by` fields |
...
| `groupBy({ by, _count/_sum/_avg/_min/_max })` | Yes | Yes | Finite-constrained `by` fields only; no `having`/`orderBy`/`skip`/`take` in v1 |
...
For rank/window-like aggregation use cases (`at/indexOf/paginate/random`), use aggregate runtime (`TableAggregate`) from `better-convex/aggregate`:
