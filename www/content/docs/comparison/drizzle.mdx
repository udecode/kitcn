...
| `count/sum/avg/max/min` in query builder | Use components | Use [Aggregates](/docs/server/advanced/aggregates) (`createAggregate`) |
...
For aggregation use cases today, use `createAggregate` from `better-convex/aggregate`:
