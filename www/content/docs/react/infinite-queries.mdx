...
export const list = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    // cursor, endCursor, and limit are automatically added to input
...
- Adds flat pagination fields to your input: `cursor`, `endCursor`, and `limit`
- Accepts the full Convex page result, including `pageStatus` and `splitCursor`

| Input Field | Type | Description |
|-------------|------|-------------|
| `cursor` | `string \| null` | Pagination cursor |
| `endCursor` | `string \| null \| undefined` | Reactive page boundary controlled by the infinite-query hook |
| `limit` | `number` | Items to fetch (capped at server limit) |

<Callout icon={<InfoIcon />}>
**Important:** Always call `.paginated({ limit, item })` before `.query()`. Pass `input.cursor`, `input.endCursor`, and `input.limit` to the matching ORM `findMany()` fields so reactive page splits stay bounded.
</Callout>
...
- Oversized reactive pages split into adjacent, boundary-pinned subscriptions
- An invalid loaded-page cursor resets the list cleanly to its first page
