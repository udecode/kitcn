...
It also exports `defineAuth` for `auth.ts` authoring and includes Better Auth runtime wiring when `auth.ts` has a default export.
Use `.create()` directly, then chain `.meta()`/middleware as needed.
...
`better-convex dev` / `better-convex codegen` generates `convex/functions/generated.ts`.
Use it as the canonical server contract and import `initCRPC` from it in `convex/lib/crpc.ts`.
...
When using generated `initCRPC` and `relations`, ORM context is already wired.
If you need manual setup (no codegen), attach the ORM with `.context()`:
