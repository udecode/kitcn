...
Shared middleware keeps mutation writer types when you apply it to a mutation chain.
If the middleware itself performs writes, make it mutation-only:

```ts showLineNumbers {1-2,5-7}
