...
<Callout icon={<InfoIcon />}>
  **Note:** Actions can't access the database directly. Module runtime callers (like `createUserCaller(ctx)`) dispatch via `ctx.runQuery` / `ctx.runMutation` under the hood, ensuring all database operations are transactional. See [In-Process Procedure Composition](/docs/server/procedures#in-process-procedure-composition) for the full call matrix.
</Callout>
