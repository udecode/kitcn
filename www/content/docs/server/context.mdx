...
Actions have a different context. Instead of direct database access, they call other procedures. Use `createCaller(ctx)` for type-safe dispatch with full autocomplete:
...
export const processAndSave = publicAction
  ...
  .action(async ({ ctx, input }) => {
    ...
    await caller.user.updateProfile({ data: await result.json() });
  });
```

<Callout icon={<InfoIcon />}>
  **Note:** Actions can't access the database directly. `createCaller(ctx)` dispatches via `ctx.runQuery` / `ctx.runMutation` under the hood, ensuring all database operations are transactional. See [In-Process Procedure Composition](/docs/server/procedures#in-process-procedure-composition) for the full call matrix.
</Callout>
