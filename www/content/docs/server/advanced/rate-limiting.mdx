...
In this guide, we'll set up `better-convex/ratelimit` with an Upstash-parity API, Convex-first storage, and a UI-friendly hook.
...
`better-convex/ratelimit` is designed as a hard cutover from component-driven APIs.
...
You can also use the pre-defined constants from `better-convex/ratelimit`:

```ts showLineNumbers
import { SECOND, MINUTE, HOUR, DAY, WEEK } from 'better-convex/ratelimit';
...
import { Ratelimit } from 'better-convex/ratelimit';
...
| `prefix` | `string` | `'@better-convex/ratelimit'` | Namespaces stored state in the database. Use unique prefixes for different rate limit scopes. |
...
Pre-defined millisecond constants exported from `better-convex/ratelimit`:
...
The rate limiter stores state in three Convex tables. These are automatically injected by the ORM's `defineSchema` — do not define tables with these names in your schema.
