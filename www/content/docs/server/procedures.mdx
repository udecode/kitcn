...
When one procedure needs to call another, use the module runtime factory from `convex/functions/generated/<module>.runtime.ts` (for example `createTeamsCaller(ctx)`). This gives you a type-safe proxy with full autocomplete — no more manually choosing between `ctx.runQuery` and `ctx.runMutation`.

<Callout icon={<InfoIcon />}>
  **Breaking change:** runtime modules now export module-named factories (`create<Module>Caller` / `create<Module>Handler`), and `convex/functions/generated.ts` no longer exists.
</Callout>
