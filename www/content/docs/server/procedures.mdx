...
Use generated `createCaller(ctx)` when one procedure needs to call other cRPC procedures.

```ts title="convex/functions/teams.ts" showLineNumbers
import { z } from 'zod';
...
`createCaller(ctx)` behavior depends on context:
- `QueryCtx` / `MutationCtx`: invokes procedure handlers directly (`_handler`) and does **not** use `ctx.runQuery` / `ctx.runMutation`.
- `ActionCtx`: dispatches automatically via `ctx.runQuery` / `ctx.runMutation`.
...
`ActionCtx` calls are still separate Convex query/mutation transactions. Prefer a single aggregated internal query/mutation when consistency matters.

`createHandler(ctx)` remains query/mutation-only and bypasses input/middleware/output validation.
