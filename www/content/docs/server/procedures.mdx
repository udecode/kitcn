...
<Callout icon={<InfoIcon />}>
  **Note:** For mutations/actions that do not return data, omit `.output(...)`.
  With no output schema, a `void`/`undefined` response is serialized by Convex as
  `null`.
</Callout>
...
`.output()` parses the value your handler returned, before it is serialized. The handler returns the schema's **input** type and clients receive its output type, so a schema that transforms applies on the way out.

Nothing is substituted for a missing value. A nullable schema needs an explicit `null`:

```ts title="convex/functions/user.ts"
// Rejected - the schema is parsed against `undefined`
export const displayName = publicQuery
  .input(z.object({ id: z.string() }))
  .output(z.string().nullable())
  .query(async ({ ctx, input }) => names[input.id]);

// Accepted
export const displayName = publicQuery
  .input(z.object({ id: z.string() }))
  .output(z.string().nullable())
  .query(async ({ ctx, input }) => names[input.id] ?? null);
```

Use `.output(z.string().default('...'))` to substitute a value instead.

TypeScript rejects most of these before they run. The case it misses is a lookup it believes is always present: under the default `noUncheckedIndexedAccess: false`, an index signature or `array[0]` types as non-`undefined` even when the value is absent. Enable `noUncheckedIndexedAccess` to close that gap. `ctx.orm.query.*.findFirst` returns `| null`, so ORM reads are unaffected.

A mismatch throws a `CRPCError` with code `INTERNAL_SERVER_ERROR`. Sanitized
structural Zod issues are available in `error.data.ZodError`; custom messages
and fields remain in the server-side error cause because they can contain the
rejected handler output.

<Callout icon={<InfoIcon />}>
  **Note:** Model an absent value as `.nullable()`, not as a top-level
  `.optional()`. Convex serializes an `undefined` return as `null` and its
  returns validator cannot express top-level optionality, so
  `.output(z.string().optional())` publishes `v.string()` and the deployment
  rejects the `null` it receives. `.optional()` inside an object is fine.
</Callout>

<Callout icon={<InfoIcon />}>
  **Note:** The low-level `returns:` option on `zCustomQuery`/`zCustomMutation`/
  `zCustomAction` does not share this contract — it substitutes `null` for an
  `undefined` return before parsing. Prefer `.output()`.
</Callout>
...
- **`QueryCtx` / `MutationCtx`**: invokes the procedure handler directly — same transaction, no Convex round trip.
...
`caller.actions.*` is action-only. If a callback can run from mutation or
action context, keep the seam honest and schedule the action:

```ts title="convex/functions/jobs.ts" showLineNumbers {1,4}
...
For large datasets, use `.paginated()` for cursor-based pagination. It adds `cursor`, `endCursor`, and `limit` to your input and accepts Convex split metadata in the output.
...
The handler receives flat `input.cursor`, `input.endCursor`, and `input.limit`. Pass all three to `findMany()`. The output accepts `{ continueCursor, isDone, page, pageStatus?, splitCursor? }` so infinite-query clients can maintain reactive page boundaries.
