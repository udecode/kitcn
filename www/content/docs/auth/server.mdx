...
In this guide, you'll install Better Auth through Better Convex's CLI-first
...
Use `npx better-convex init --yes` instead when you are adopting the current
supported app.
...
The default Better Convex auth path writes and patches the auth surface for you:
...
`functionsDir` comes from `convex.json.functions`. Scaffolded Better Convex
...
The default Better Convex path patches auth-owned table blocks directly into
...
tables change, rerun the same preset command. It does not support
`--only schema`.
...
`better-convex init --yes`, `better-convex dev`, and
`better-convex add auth --yes` already drive the local auth bootstrap when they
own the flow.
...
<Callout icon={<InfoIcon />}>
**Troubleshooting:** If you see "Invalid token signature" errors after deployment, re-run `better-convex env push --prod`.
</Callout>
...
```bash
npx better-convex env push --rotate --prod
```
