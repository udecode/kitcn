...
Initialize the deployment, then generate the runtime once:
...
Now we'll create the heart of your auth setup: the auth contract. This is a single file that defines everything about your authentication — plugins, session settings, trusted origins, and user triggers. You export it as a default export using `defineAuth`, and `better-convex` codegen takes care of the rest.

Here's what a typical auth contract looks like:

```ts title="convex/functions/auth.ts" showLineNumbers {3,6-11,13-22}
...
- **`convex()` plugin** — connects Better Auth to your Convex database using the auth config and static JWKS from Step 1.
- **`admin()` plugin** — adds admin capabilities like user banning and role management.
- **`session`** — controls how long sessions last and how often they refresh.
- **`trustedOrigins`** — restricts which origins can make auth requests, preventing CSRF attacks.
- **`triggers`** — hooks that run during auth lifecycle events.
...
This pushes `convex/.env` values to the active deployment.
...
<Tabs groupId="triggers" items={["Schema triggers (recommended)", "defineAuth triggers"]} persist>
  ...
  <Tab value="defineAuth triggers">
    ...
    ```ts title="convex/functions/auth.ts" showLineNumbers {4-14}
    import { defineAuth } from './generated/auth';
...
| `JWKS` | Auto-generated by `env push` |
| `BETTER_AUTH_SECRET` | Auto-generated by `env push` |
...
Run `npx better-convex env push` to push all variables to Convex.
...
```bash
npx better-convex env push --auth --prod
```
...
<Callout icon={<InfoIcon />}>
**Troubleshooting:** If you see "Invalid token signature" errors after deployment, re-run `better-convex env push --auth --prod`.
</Callout>
...
```bash
npx better-convex env push --auth --rotate --prod
```
