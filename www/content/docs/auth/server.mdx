  ...
  return {
    ...
    triggers: {
      user: {
        ...
        onCreate: async (user) => {
          // `ctx` from defineAuth closure is available here.
          // Example: const caller = createModuleCaller(ctx); await caller.schedule.now.fn(args);
          void user;
          void ctx;
        },
      },
...
If you also need DB-level side effects (aggregates, cascades, denormalized counters), define schema-level triggers in `schema.ts`:
...
When your schema exports `relations`, generated runtime automatically wires ORM context so auth handlers can use `ctx.orm` directly.
