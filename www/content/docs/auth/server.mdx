...
Now define `auth.ts` as a single default export. `better-convex` codegen will generate the runtime (`getAuth`, CRUD handlers, trigger handlers, and static `auth`) in `convex/functions/generated.ts`.
...
<Callout icon={<InfoIcon />}>
Run `better-convex dev` or `better-convex codegen` first, then create `auth.ts` using `defineAuth` from `./generated`, then re-run codegen. Runtime exports live in `convex/functions/generated.ts` and include `authEnabled` for setup checks.
</Callout>
