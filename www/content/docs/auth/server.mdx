      ...
      user: {
        create: {
          ...
          after: async (user, triggerCtx) => {
            // `ctx` from defineAuth closure is still available here.
            // `triggerCtx` is also passed explicitly to every callback.
          },
        },
      },
    },
...
Auth triggers use the same API as [ORM Triggers](/docs/orm/schema/triggers) — see that page for the full hook signatures.

<Callout icon={<InfoIcon />}>
`defineTriggers` runs at the `ctx.orm` level (all writes). `defineAuth` triggers run only at the Better Auth API level.
</Callout>

<Tabs groupId="triggers" items={["ORM (recommended)", "defineAuth"]} persist>
  <Tab value="ORM (recommended)">
    Define triggers in `schema.ts` via `defineTriggers`. These run for all writes to auth tables, including ORM mutations:

    ```ts title="convex/functions/schema.ts" showLineNumbers {1-2,4-16}
  ...
  <Tab value="defineAuth">
    Define triggers inline in `auth.ts`. These run only for auth CRUD operations:

    ```ts title="convex/functions/auth.ts" showLineNumbers {4-16}
    import { defineAuth } from './generated/auth';
