...
With `better-convex dev` running, sync to Convex:
...
If no deployment is active, start `npx better-convex dev` first, then retry.
...
```bash
npx convex run generated/auth:rotateKeys | npx convex env set JWKS
```
...
```bash
npx convex run generated/auth:getLatestJwks --prod | npx convex env set JWKS --prod
```
...
```bash
npx convex run generated/auth:rotateKeys --prod | npx convex env set JWKS --prod
```
