...
<PackageInstallTabs packages="@polar-sh/better-auth @polar-sh/sdk buffer" />
...
import { createAuthClient } from 'kitcn/auth/client';
...
```ts title="convex/functions/polarCustomer.ts" showLineNumbers {1,3-8,11-18,20-31}
import '../lib/polar-polyfills';
...
See the [example app](https://github.com/nichochar/kitcn/tree/main/example) for complete subscription CRUD implementations.
...
```ts showLineNumbers title="convex/functions/auth.ts"
webhooks({
  secret: process.env.POLAR_WEBHOOK_SECRET!,
