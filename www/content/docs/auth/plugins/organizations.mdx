...
- Use `ctx.auth.api.*` for multi-table operations (create, delete, invitations, member management) when they stay inside Convex.
- Use `ctx.orm` for simple reads and updates
- Use `authAction` for Better Auth endpoints that can run external plugin work such as Stripe, Polar, or direct email delivery. Convex mutations cannot call those SDKs.
