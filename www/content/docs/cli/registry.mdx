...
| Goal | Command |
| ---- | ------- |
| Start a new Expo app | `kitcn init -t expo --yes` |
| Start a new Next.js app | `kitcn init -t next --yes` |
| Start a new TanStack Start app | `kitcn init -t start --yes` |
...
Pass `-t next`, `-t expo`, `-t start`, or `-t vite` when you want kitcn to create the app shell
...
For a fresh Expo app, `init -t expo` uses the official `create-expo-app`
default shell, then overlays the Convex client/provider wiring plus one native
messages screen. It is still fresh scaffold only for app ownership, but Expo
does support the default `add auth` scaffold on top of that baseline. Existing
Expo app adoption is still out of scope.
...
| Flag | Description |
| ---- | ----------- |
| `-t, --template <next\|expo\|start\|vite>` | Fresh app template |
