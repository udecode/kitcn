...
| Verify local runtime | `better-convex verify` |
...
or auth fields in `<functionsDir>/auth.ts`, use `add auth --only schema --yes`
...
- `--only schema` does not apply; re-run the full preset command when that auth
  schema changes
...
| `--only schema` | Refresh only the managed auth schema scope |
