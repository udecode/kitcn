...
`kitcn auth jwks` runs the generated auth runtime and prints a manual static
`JWKS` payload. Use it when you want static JWKS without going through
`kitcn env push`, especially on backend `concave` where `kitcn env` has no
upstream env command to wrap.

Target flags are forwarded to the backend `run` seam:

- Convex: `--prod`, `--deployment-name`, `--preview-name`, `--env-file`, `--component`
- Concave: `--url`, `--port`, `--component`
