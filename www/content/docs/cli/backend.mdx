...
On backend `convex`, `better-convex dev` runs `convex init` before codegen,
watchers, and the main `convex dev` process.

Set `meta["better-convex"].dev.preRun` in `concave.json` to run one Convex
...
npx better-convex dev --bootstrap
```

## verify

```bash showLineNumbers
npx better-convex verify
```

Runs a one-shot local Convex runtime proof through the Better Convex dev path.

Use it when you want a CI-safe answer to one question: does local Better Convex
...
npx better-convex env push --rotate
...
`better-convex env push` reads `convex/.env` by default, filters Convex-managed variables, then pushes one batch update through `convex env set --from-file`. If auth scaffold is installed, it also ensures `BETTER_AUTH_SECRET` exists locally, fetches `JWKS` with `convex run generated/auth:getLatestJwks`, and includes both in the push payload. With `--rotate`, it runs `convex run generated/auth:rotateKeys` first, then pushes the fresh `JWKS`.

On backend `convex`, `better-convex dev` handles the local auth bootstrap flow and watches `convex/.env` for later local edits. `better-convex dev --bootstrap` is the one-shot local setup version of that flow. `better-convex verify` is the one-shot local runtime proof. `better-convex add auth --yes` reuses the same local bootstrap path when it needs generated auth runtime and JWKS. Use `better-convex env push` for `--prod`, `--rotate`, or explicit repair against an already active deployment.
