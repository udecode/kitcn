...
1. `convex deploy ...`
2. post-deploy `generated/server:aggregateBackfill`
3. status polling (`generated/server:aggregateBackfillStatus`) until `READY` (default behavior)
...
The `--yes` flag is required — reset is destructive. Under the hood it calls `generated/server:reset` (which uses `ctx.orm.withoutTriggers` to clear every table).
