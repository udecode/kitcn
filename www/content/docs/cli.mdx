...
- `better-convex codegen` discovers plugins from `defineSchema(..., { plugins })`.
...
Preview scaffold drift without writing files:

```bash showLineNumbers
npx better-convex diff <plugin>
```

If `<plugin>` is omitted in a TTY, the CLI prompts you to choose from installed plugins.

Template selection for `diff` is deterministic:
...
- Passing `--preset` diffs against that preset template set.
...
| `--preset <name>` | Plugin preset override for diff template set |
| `--verbose-diff` | Print unified patch text for changed/missing files |

### list

Show installed plugin inventory from schema + lockfile:
