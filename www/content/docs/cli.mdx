...
| Plugin install plans | Preview scaffold, schema, lockfile, dependency, and hook effects before writing |
| Project inspection | Audit installed plugins, drift, and docs links |
...
| Backend selection | Drive Convex or Concave with one CLI |
...
Bootstrap Better Convex into the current app, or opt into shadcn-backed app creation.
...
- `better-convex init` writes the Better Convex baseline only:
  ...
  - `concave.json`
  - a first generated runtime pass for `convex/functions/generated/server.ts`
- `better-convex init -t next` creates a shadcn Next.js app in the resolved target directory, then adds the Better Convex integration layer and patches the existing shell minimally:
  - `package.json`
  - `tsconfig.json`
  - `.env.local`
  - `app/convex/page.tsx`
  ...
  - the Better Convex baseline files above, with `convex/functions/schema.ts`
    seeded with a starter `messages` table
- template-mode init preserves shadcn-owned files such as `app/layout.tsx`, `app/page.tsx`, `app/globals.css`, `components/theme-provider.tsx`, `lib/utils.ts`, `components.json`, `eslint.config.mjs`, `next.config.mjs`, and `postcss.config.mjs`, and only patches:
...
- template-mode init detects `src/` vs root app layouts and writes the client scaffold into the matching tree. Mixed `src` + root layouts fail fast instead of guessing.
- init tries real Better Convex codegen immediately. If the app is not linked to a Convex deployment yet, init first tries anonymous local bootstrap. For explicit project-backed bootstrap in non-interactive environments, pass `--team`, `--project`, and optionally `--dev-deployment`. Plain non-template init can fall back to a stubbed `convex/functions/generated/server.ts` only when real codegen still cannot complete; `init -t next` fails instead.
- `--yes` makes the Next.js path deterministic for CI by using the pinned shadcn bootstrap and its default answers.
- `--cwd` picks the target directory. When you also pass `--name`, the final app path is `<cwd>/<name>`.
- `--json` includes `codegen` (`generated` or `stubbed`) and `convexBootstrap` (`existing`, `created`, or `missing`) for automation.
- `better-convex create` is an alias for `init`.
...
| Flag | Description |
|------|-------------|
| `-t, --template <template>` | Project template to create before overlaying Better Convex. Only `next` is supported. |
| `--backend <convex\|concave>` | Backend CLI to drive |
| `--cwd <cwd>` | Target directory |
| `--name <name>` | Name for a new target directory |
| `--team <slug>` | Convex team slug for non-interactive project-backed bootstrap |
| `--project <slug>` | Convex project slug for non-interactive project-backed bootstrap |
| `--dev-deployment <cloud\|local>` | Convex dev deployment kind for init bootstrap (default: `local`) |
...
Other env subcommands pass through to Convex CLI only:
...
On backend `concave`, raw `better-convex env ...` passthrough fails clearly because Concave has no upstream env command.
