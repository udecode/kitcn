---
title: Quickstart
description: Scaffold a Next.js app with Better Convex, bootstrap a local backend, and understand the starter messages demo.
links:
...
In this guide, we'll take the fastest happy path: scaffold a fresh Next.js app
with the CLI, run the first local Convex bootstrap in the same command, and
walk through the starter messages demo from schema to client.
...
`init -t next` creates the Next.js shell and layers in the Better Convex
...
If you already have a supported app shell, use `better-convex init --yes`
instead. That adoption path is documented in
[/docs/cli/registry](/docs/cli/registry).

## 2. Run The App

Now let's start the backend for real and open the Next.js app in a second
terminal.
...
`better-convex dev`. The starter page already includes that empty/error state,
  ...
  return (
    <main>
      ...
      {messagesQuery.isPending ? (
        ...
        <div>Backend not ready. Start better-convex dev and refresh.</div>
      ) : (
        <ul>
...
You now have a running Better Convex app, a local deployment, generated API
types, and one live page wired end to end. That's the real starting point.
