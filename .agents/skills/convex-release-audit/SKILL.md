...
When this audit needs durable goal state, use the project-owned Convex release
audit template instead of the generic task template:
...
This plan owns version discovery, changelog reconciliation, upstream diff
evidence, kitcn leverage classification, the selected slice or no-action
verdict, and the delegated task prompt. Implementation still goes through
`task`; do not duplicate implementation PR machinery inside the release-audit
plan.
...
- Keep the PR slice coherent. One release opportunity per PR unless multiple
  fixes share the same implementation boundary.
