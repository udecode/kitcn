...
4. If the task comes from a tracker item and any attachment or linked media is a
...
- Do not add decorative metadata like `title` to cached
...
- For auth-gated tracker media like `uploads.linear.app` or private GitHub asset
...
5. If there are multiple videos, preserve each as its own
...
8. If the task is heavyweight work:
   ...
   - treat `major-task` as the source of truth for workflow and helper
     selection
   - do not quietly inflate this task flow into a research swarm
9. If the task is not heavyweight work, classify task complexity before
   implementation:
...
- if the task touches published package code under `packages/`, record in the
  plan whether the active unreleased changeset must be updated before completion
- if the task changes `kitcn init -t` templates or scaffold sources, record
...
- do not reuse an unrelated non-`main` branch just because you are already on
  it
- if the current branch is not clearly dedicated to this task, check out `main`,
...
18. If anything important is still ambiguous after the source-of-truth pass and
...
2. If behavior-level coverage is sane, turn the repro into the smallest useful
   automated test and watch it fail for the right reason.
3. Find the real ownership boundary of the bug. Prefer fixing the abstraction,
   contract, or API that caused it instead of patching each caller around it.
4. If the best fix requires an API change, make it unless the task source or
   repo constraints explicitly rule that out. Do not preserve a bad API just to
   keep the diff small.
5. If you intentionally choose a narrower fix, state why the broader
...
3. Prefer the cleanest long-term design that fits the slice, not the quickest
   bolt-on.
4. If existing patterns are the reason the design is weak, improve the pattern
...
3. If the task exposes a bad API or abstraction, fix that seam instead of
   polishing around it.
4. Run the narrowest regression checks plus the relevant build, typecheck, or
...
2. For review tasks, report findings first, ordered by severity, with concrete
   file references.
3. For investigation tasks, identify the failure mode, probable cause, and next
...
- If a failing `bun test`, `bun check`, or `bun typecheck` shows
  ...
  - package-local `node_modules/react` or `node_modules/react-dom` paths under
    `packages/*`
...
- If verified work changes published package code under `packages/`, ensure the
...
- If the work changes `kitcn init -t` templates or scaffold sources, run
...
- use these exact status values in the flow table:
  ...
  - `❌`
  - `➖ N/A`
  - prefer specific browser caveat text over vague `⚠️ Partial`, for example
...
- use `➖ N/A` for rows or cells that do not apply; do not invent a PR, issue,
  or comment
...
- if browser verification applies, `**🌐 Browser Check**` must include the exact
...
- When a PR exists, the PR description must match the exact current final
  handoff from chat:
  - same flow table
  - same metadata lines, except omit the leading `🔀 PR ...` line because the
    PR page already identifies itself
  - same screenshot when applicable
  - same `**✅ Outcome**`, `**🏗️ Design**`, `**🧪 Verified**`, `**⚠️ Caveat**`,
...
- Video attachments or screen recordings were turned into normalized
  ...
  required it.
- Bare GitHub issues like `#555` were resolved against the current `gh` repo
  instead of guessed.
...
- Code-changing tasks that did not already start on a branch clearly dedicated
  to the same task checked out `main` and pulled latest before branching.
- Non-trivial work loaded `planning-with-files` or the repo-equivalent planning
...
- Non-trivial package or scaffold work recorded release-artifact requirements in
  ...
  fixture sync/check for `kitcn init -t` template or scaffold work.
