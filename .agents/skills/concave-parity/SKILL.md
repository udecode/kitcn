...
Concave still has no equivalent upstream env command surface. That means auth
runtime parity exists, but env-management parity does not.

kitcn workaround:

- expose `kitcn auth jwks`
- expose `kitcn auth jwks --rotate`
- print a manual `JWKS=...` payload instead of inventing fake env passthrough

Code locations:

- `packages/kitcn/src/cli/commands/auth.ts`
- `packages/kitcn/src/cli/commands/auth.test.ts`
- `packages/kitcn/src/cli/cli.ts`
- `packages/kitcn/src/cli/cli.commands.ts`

Why it exists:

- users still need a manual static `JWKS` path on backend `concave`
- `kitcn env` is correctly Convex-only today

Delete when:

- Concave exposes a stable env management seam kitcn can wrap directly, or
- kitcn ships a real cross-backend env abstraction and no longer needs a
  JWKS-specific manual helper
